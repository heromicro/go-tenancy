...
func CreateMini(ctx *gin.Context) {
	...
	if id, err := service.CreateMini(mini); err != nil {
...
func UpdateMini(ctx *gin.Context) {
	...
	if err := service.UpdateMini(req.Id, mini); err != nil {
...
func GetMiniById(ctx *gin.Context) {
	...
	mini, err := service.GetMiniByID(req.Id)
	if err != nil {
		g.TENANCY_LOG.Error("获取失败!", zap.Any("err", err))
		response.FailWithMessage("获取失败:"+err.Error(), ctx)
	} else {
...
func DeleteMini(ctx *gin.Context) {
	...
	if err := service.DeleteMini(req.Id); err != nil {
